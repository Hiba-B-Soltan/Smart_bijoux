...
QSqlQueryModel * Produit:: afficher_produit()
{
    ...
    model->setHeaderData(4,Qt::Horizontal,QObject::tr("MATERIAUX"));
...
QSqlQueryModel* Produit ::recherche(const QString& searchText)
{
    QSqlQueryModel *model = new QSqlQueryModel();
       QSqlQuery query;

       QStringList columns;  // List of all columns in your table
       columns << "id_prduit" << "quantite" << "prix" << "poids" << "materiaux" << "couleur" << "fournisseur";

       QString queryString = "SELECT * FROM produit WHERE ";

       // Generate the dynamic part of the query based on all columns
       for (const QString& column : columns) {
           queryString += QString("%1 LIKE :searchText OR ").arg(column);
       }
